...
namespace FiMa.ViewModels {

    public partial class OutcomeViewModel : ObservableObject {

        private readonly DatabaseContext _context;

        public OutcomeViewModel(DatabaseContext context) {
            _context = context;
        }

        [ObservableProperty]
        private ObservableCollection<Outcome> _products = new();

        [ObservableProperty]
        private Outcome _operatingProduct = new();
        ...
        public async Task LoadProductsAsync() {
            await ExecuteAsync(async () => {
                var products = await _context.GetAllAsync<Outcome>();
                if (products is not null && products.Any()) {
                    Products ??= new ObservableCollection<Outcome>();
        ...
        [RelayCommand]
        private void SetOperatingProduct(Outcome? product) => OperatingProduct = product ?? new();
        ...
        private async Task SaveProductAsync() {
            ...
            var busyText = OperatingProduct.IDDespesa == 0 ? "Creating product..." : "Updating product...";
            await ExecuteAsync(async () => {
                if (OperatingProduct.IDDespesa == 0) {
                    // Create product
                    await _context.AddItemAsync<Outcome>(OperatingProduct);
                    Products.Add(OperatingProduct);
                } else {
                    // Update product
                    if (await _context.UpdateItemAsync<Outcome>(OperatingProduct)) {
        ...
        private async Task DeleteProductAsync(int id) {
            await ExecuteAsync(async () => {
                if (await _context.DeleteItemByKeyAsync<Outcome>(id)) {
                    var product = Products.FirstOrDefault(p => p.IDDespesa == id);
