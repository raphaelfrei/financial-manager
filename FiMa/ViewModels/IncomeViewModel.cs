...
namespace FiMa.ViewModels {
    ...
    public partial class IncomeViewModel : ObservableObject {
        ...
        public IncomeViewModel(DatabaseContext context) {
            _context = context;
        }

        [ObservableProperty]
        private ObservableCollection<Income> _products = new();

        [ObservableProperty]
        private Income _operatingProduct = new();
        ...
        public async Task LoadProductsAsync() {
            await ExecuteAsync(async () => {
                var products = await _context.GetAllAsync<Income>();
                if (products is not null && products.Any()) {
                    Products ??= new ObservableCollection<Income>();
        ...
        [RelayCommand]
        private void SetOperatingProduct(Income? product) => OperatingProduct = product ?? new();
        ...
        private async Task SaveProductAsync() {
            ...
            var busyText = OperatingProduct.IDReceita == 0 ? "Creating product..." : "Updating product...";
            await ExecuteAsync(async () => {
                if (OperatingProduct.IDReceita == 0) {
                    // Create product
                    await _context.AddItemAsync<Income>(OperatingProduct);
                    Products.Add(OperatingProduct);
                } else {
                    // Update product
                    if (await _context.UpdateItemAsync<Income>(OperatingProduct)) {
        ...
        private async Task DeleteProductAsync(int id) {
            await ExecuteAsync(async () => {
                if (await _context.DeleteItemByKeyAsync<Income>(id)) {
                    var product = Products.FirstOrDefault(p => p.IDReceita == id);
