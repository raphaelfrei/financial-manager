...
namespace FiMa.ViewModels {
    ...
    public partial class UserViewModel : ObservableObject {
        ...
        public UserViewModel(DatabaseContext context) {
            _context = context;
        }

        [ObservableProperty]
        private ObservableCollection<User> _products = new();

        [ObservableProperty]
        private User _operatingProduct = new();
        ...
        public async Task LoadProductsAsync() {
            await ExecuteAsync(async () => {
                var products = await _context.GetAllAsync<User>();
                if (products is not null && products.Any()) {
                    Products ??= new ObservableCollection<User>();
        ...
        [RelayCommand]
        private void SetOperatingProduct(User? product) => OperatingProduct = product ?? new();
        ...
        private async Task SaveProductAsync() {
            ...
            var busyText = OperatingProduct.IDUsuario == 0 ? "Creating product..." : "Updating product...";
            await ExecuteAsync(async () => {
                if (OperatingProduct.IDUsuario == 0) {
                    // Create product
                    await _context.AddItemAsync<User>(OperatingProduct);
                    Products.Add(OperatingProduct);
                } else {
                    // Update product
                    if (await _context.UpdateItemAsync<User>(OperatingProduct)) {
        ...
        private async Task DeleteProductAsync(int id) {
            await ExecuteAsync(async () => {
                if (await _context.DeleteItemByKeyAsync<User>(id)) {
                    var product = Products.FirstOrDefault(p => p.IDUsuario == id);
